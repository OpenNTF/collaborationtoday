...
public abstract class PluginManager {
	...
	/**
	 * Loads and returns the classes defined in the properties files. If the system property "rome.pluginmanager.useloadclass" is set to
	 * true then classLoader.loadClass will be used to load classes (instead of Class.forName). This is designed to improve OSGi
	 * compatibility. Further information can be found in https://rome.dev.java.net/issues/show_bug.cgi?id=118
	 * <p>
	 * 
	 * @return array containing the classes defined in the properties files.
	 * @throws java.lang.ClassNotFoundException
	 *             thrown if one of the classes defined in the properties file cannot be loaded and hard failure is ON.
	 * 
	 */
	private Class[] getClasses() throws ClassNotFoundException {
		ClassLoader classLoader = Thread.currentThread().getContextClassLoader();

    	// added to support DOTS scheduled runs 
    	boolean workInOSGI = Boolean.valueOf(System.getProperty("rome.custom.workInOSGI", "false")).booleanValue();
    	if(workInOSGI) classLoader = Activator.class.getClassLoader();

    ...
    private static String getContentTypeEncoding(String httpContentType) {
        ...
        if (httpContentType!=null) {
            ...
            if (i>-1) {
                ...
                encoding = (encoding!=null) ? encoding.toUpperCase(Locale.US) : null; //Corrected due to dotless-i problem
//                encoding = (encoding!=null) ? encoding.toUpperCase() : null; // original
            }
            if (encoding != null &&
    ...
    private static String getXmlProlog(BufferedInputStream is,String guessedEnc) throws IOException {
        ...
        if (guessedEnc!=null) {
            ...
            if (bytesRead>0) {
                ...
                if (m.find()) {
                    encoding = m.group(1).toUpperCase(Locale.US); //Corrected due to dotless-i problem
                    //encoding = m.group(1).toUpperCase(); // Original 
                    encoding = encoding.substring(1,encoding.length()-1);
                }
            }
        }

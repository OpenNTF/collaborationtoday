...
//    private static final String MASTER_PLUGIN_FILE = "com/sun/syndication/rome.properties";
    ...
    private PropertiesLoader(String masterFileLocation,String extraFileLocation) throws IOException {
        ...
        try {
        	...
        	InputStream is = null;
        	if(workInOSGI) {
        		is = getOSGIResource(masterFileLocation);
        	} else {
        		is = classLoader.getResourceAsStream(masterFileLocation);
        	}
        	
//        	InputStream is = classLoader.getResourceAsStream(masterFileLocation); // Removed to support DOTS Scheduled Runs
	...
	// added to support DOTS scheduled runs 
	private static InputStream getOSGIResource(String path) {
		InputStream result = null;
		URL url;
		try {
			url = new URL("platform:/plugin/" + Activator.PLUGIN_ID + "/" + path);

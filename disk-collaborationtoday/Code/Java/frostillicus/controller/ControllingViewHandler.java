...
public class ControllingViewHandler extends com.ibm.xsp.application.ViewHandlerExImpl {
	...
	public UIViewRoot createView(FacesContext context, String pageName) {
		...
		if(pageClassName.equalsIgnoreCase("$$OpenDominoDocument")) {
			...
			try {
				...
				if(doc != null) {
					...
					String formUNID = strRightBack(strLeftBack(formURL, "?Open"), "/");
					...
					if(StringUtil.isEmpty(xpageAlt)) {
						...
						pageClassName = strLeftBack(xpageAlt, ".xsp");
					}
				}
		...
		try {
			...
			Map<String, Object> requestScope = (Map<String, Object>)resolveVariable("requestScope");
			requestScope.put("pageController", pageController);
			...
			viewScope.put("pageController", pageController);
			requestScope.remove("pageController");

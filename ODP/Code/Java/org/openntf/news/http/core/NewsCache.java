...
public class NewsCache {
	...
	private ArrayList<NewsEntry> getTypedEntriesList(final String tID) {
	...
	private ArrayList<NewsEntry> getCategorizedTopEntriesList(final String cID) {
		ArrayList<NewsEntry> newsEntries = null;
		newsEntries = (ArrayList<NewsEntry>) _categorizedTopNewsEntries.get(cID);
		if (newsEntries == null) {
			newsEntries = new ArrayList<NewsEntry>();
	...
	private synchronized void init() {
		...
		ConfigCache config = (ConfigCache) context.getApplication().getVariableResolver().resolveVariable(context, "configCache");
		...
		try {
			...
			while (entry != null) {
				try {
					...
					NewsEntry newsEntry = new NewsEntry((String) columnValues.get(8), (String) columnValues.get(1), (String) columnValues.get(2), (String) columnValues.get(3), (String) columnValues
							.get(5), (String) columnValues.get(6), (String) columnValues.get(7), d1, d2, (String) columnValues.get(9), (String) columnValues.get(10), (String) columnValues.get(11),
							(String) columnValues.get(12), clicksTotalDouble, clicksLastWeekDouble, d3, (String) columnValues.get(16), (String) columnValues.get(17), spotlightImageURL,
							(String) columnValues.get(19),

							// moderator
							(String) columnValues.get(20),

							(String) columnValues.get(21), d4);

					_newsEntries.add(newsEntry);
					if (newsEntry.isSpotlight())
						_spotlightNewsEntries.add(newsEntry);

					// multiple TIDs support
					if (newsEntry.getTID().indexOf(";") == -1) {
						getTypedEntriesList(newsEntry.getTID()).add(newsEntry);
					} else {
						tids = newsEntry.getTID().split(";");
						for (String tid : tids) {
							getTypedEntriesList(tid.trim()).add(newsEntry);
						}
					}
			...
			if (_categorizedTopNewsEntries != null) {
				...
				if (categories != null) {
					...
					for (; it.hasNext();) {
						...
						_categorizedTopNewsEntries.put(category.getID(), catTopNews);
					}
				}
			}
		} catch (Exception e) {
	...
	public List<NewsEntry> getEntriesByPopularity() {
		...
		for (int i = 0; i < _newsEntries.size(); i++) {
			...
			NewsEntry copy = new NewsEntry(n.getID(), n.getTID(), n.getTitle(), n.getPID(), n.getLink(), n.getImageURL(), n.getAbstract(), n.getModerationDate(), n.getPublicationDate(), n
					.isSpotlight(), n.isTopStory(), n.getTopStoryCategory(), n.getTopStoryPosition(), n.getClicksTotal(), n.getClicksLastWeek(), n.getCreationDate(), n.getState(), n
					.getSpotlightSentence(), n.getSpotlightImageURL(), n.getSpotlightPosition(), n.getModerator(), n.getLastEditor(), n.getLastModified());
	...
	public List<NewsEntry> getEntries(final String pID) {
		initialize();
	...
	public List<NewsEntry> getEntriesByType(final String tID, final String pID) {
		initialize();
		if (pID==null || pID.equals("")) {
			return getTypedEntriesList(tID);
		}
		List<NewsEntry> entries = new ArrayList<NewsEntry>();
		for (NewsEntry ent : getTypedEntriesList(tID)) {
			if (ent.getPID().equals(pID)) {
				entries.add(ent);
			}
		}
	...
	public NewsEntry getSpotlightEntry(final int position) {
	...
	public List<NewsEntry> getTopStories(final String cID) {
		initialize();
	...
	private List<NewsEntry> sortTopStories(final List<NewsEntry> newsEntries) {
	...
	private List<NewsEntry> sortSpotlightStories(final List<NewsEntry> newsEntries) {
		List<NewsEntry> output = new ArrayList<NewsEntry>();
		for (int i = 0; i < newsEntries.size(); i++) {
			NewsEntry newsEntry = newsEntries.get(i);

...
public class ConfigCache {
	...
	public void setCaptchaPublicKey(final String captchaPublicKey) {
	...
	public void setCaptchaPrivateKey(final String captchaPrivateKey) {
		...
		try {
			...
			while (entry != null) {
				if (entry.isCategory() == false) {
					...
					if (moderatorsItem != null)
						moderators = moderatorsItem.getValues();
					Type type = new Type((String) columnValues.get(2),
							(String) columnValues.get(3), moderators, (String) columnValues.get(4),
							(String) columnValues.get(0), (String) columnValues.get(5));
					_types.add(type);
				}
			...
			while (entry != null) {
				if (entry.isCategory() == false) {
					...
					Category category = new Category((String) columnValues.get(1),
							(String) columnValues.get(2), (String) columnValues.get(3));
			...
			MiscUtils.incinerate(viewNavigatorTypes, viewNavigatorCategories, viewTypesAll,
					viewConfigAll, viewCategoriesAll);
		}
		_isCached = true;
	}
	...
	public Type getType(final String tID) {
		initialize();
		if (_types == null)
		...
		if (tID.indexOf(";") != -1) {
			...
			for (Type type : _types) {
				...
				if (type.getID().equalsIgnoreCase(tID.split(";")[0]))
					return type;
			}
		} else {
			for (Type type : _types) {
				if (type.getID().equalsIgnoreCase(tID))
					return type;
			}
		}
		return Type.getEmptyType();
	...
	public Category getCategory(final String cID) {
		...
		if (cID.indexOf(";") != -1) {
			...
			for (Category cat : _categories) {
				if (cat.getID().equalsIgnoreCase(cID.split(";")[0]))
					return cat;
			}
		} else {
			...
			for (Type type : _types) {
				...
				if (moderators != null) {
					if (moderators.contains(userName)) {
						...
						if (category != null)
							output.add(category.getDisplayName() + " - " + type.getDisplayName()
									+ "|" + type.getID());
					}
				}
			}
	...
	public Vector<Type> getTypesForCategory(final String categoryId) {
		initialize();
	...
	public boolean isUserAvatarModerator(final String user) {
		if (_avatarModerators == null)
			_avatarModerators = new HashMap<String, Boolean>();
	...
	public boolean isUserModerator(final String user) {
		if (_moderators == null)
			_moderators = new HashMap<String, Boolean>();
	...
	public boolean isUserTopStoriesModerator(final String user) {
		if (_topStoriesModerators == null)
			_topStoriesModerators = new HashMap<String, Boolean>();
	...
	public boolean isUserSpotlightModerator(final String user) {
		if (_spotlightModerators == null)
			_spotlightModerators = new HashMap<String, Boolean>();
	...
	private boolean hasUserRole(final String user, final String roleName) {
	...
	@SuppressWarnings("unchecked")
	private List<String> getUserRoles(final String user) {
